...
public:
    ...
    // ============================================
	// Set overall process tree from syntax string
    inline void AddProcess(const std::string &syntax) {
        process_tree_.AddProcess(syntax);
    }

    // Set visible and missing ET momentum inputs
    ...
    inline void SetInvisibleSubsystemMomenta(int index, double val_x, double val_y) 
	{
...
protected: //rev:no inline
    ...
    void MinimizeCombined(ROOT::Minuit2::MnUserParameters &params);
    void MinimizeMigrad(ROOT::Minuit2::MnUserParameters &params);
    void MinimizeSimplex(ROOT::Minuit2::MnUserParameters &params);
};
}  // namespace OptiMass
