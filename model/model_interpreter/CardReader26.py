...
class CardReader():
    ...
    group_function_rules = {
            ...
            "mean" : "CalcMean" ,
            "Mean" : "CalcMean" ,
            "GeomAvg" : "CalcGeometricMean" , 
            "GeomMean" : "CalcGeometricMean" , 
            "HarmonicAvg" : "CalcHarmonicMean" ,
            "HarmonicMean" : "CalcHarmonicMean" ,
            "rms" : "CalcRMS",
            "RMS" : "CalcRMS"
        }
...
#        print("reading card from input file")
    ...
    def __interpret_particle_label(self):
        for elem_ptl in self.elem_tree.getiterator("Particle"):
            for ptl_label in self.elem_particle_label.findall("Particle"):
    ...
    def __generate_effective_tree(self):        
        ...
        for elem_ptl in self.elem_tree_effective.getiterator("Particle"):
            for ptl_label in self.elem_particle_label.findall("Particle"):
                if ptl_label.get("label") != None and ptl_label.get("label") == elem_ptl.get("label"):
                    elem_ptl.attrib.update(ptl_label.attrib)
        for elem in self.elem_tree_effective.getiterator("Particle"):
            if elem.get("optimize_target") == "True":
                invisible_found = False
                for elem_ptl_sub in elem.getiterator("Particle"):
    ...
    def __interpret_mass_elem(self,elem_input):
        ...
        if elem_input.tag == "Particle":        
            output = "new MassFunctionParticle( dynamic_cast<MassMinimizer*>(this), \"" + elem_input.get("label") +"\", &" + self.mass_function_rules[elem_input.get("mass_function")] + " )"
            ...
            output = "new MassFunctionGroup( dynamic_cast<MassMinimizer*>(this) , &" + self.group_function_rules[elem_input.get("group_function")] + " , {\n"
...
#        for ptl in self.elem_particle_target.findall("Particle"):
        for ptl in self.elem_particle_target.getiterator("Particle"):
            output.append(ptl.attrib["label"])
        return output
    ...
    def get_optimize_particles(self):
        ...
        for elem_ptl in self.elem_tree.getiterator("Particle"):
            ...
            if(elem_ptl.find("Particle") != None and elem_ptl.get("optimize_target") == "True"):
                invisible_found = False
                for elem_ptl_sub in elem_ptl.getiterator("Particle"):
                    if elem_ptl_sub.find("Particle") == None and elem_ptl_sub.get("invisible") == "True" and not invisible_found:
    ...
    def get_invisible_subsystem(self):
        ...
        #                for elem_final in elem_on_tree.getiterator("Particle"):
        ...
        for elem in self.elem_constraints.findall("Constraint"):
            for rule in self.postfix_rules:
                for cmd in re.findall('([a-zA-Z0-9]+' + rule[0].replace('.','\.').replace('(','\(').replace(')','\)') + ')',elem.text.strip()):
                    output.add(rule[2] + ' ' + cmd.replace('.','_').rstrip('()') + ' = ' + rule[1] + '(\"' + cmd.replace(rule[0],'') + '\")' )
        return list(output)
    ...
    def print_code(self):
        for elem in self.elem_card.getiterator("code"):
