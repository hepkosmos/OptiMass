...
+--------------------+-----------------------+
          < Model information >
+--------------------------------------------+
#1) Model Name : 'Zprime_ww_mixed'
+--------------------------------------------+
#2) Full Decay Topology :
+--------------------------------------------+
    chain-(1) : Zp - n1 n2 , ( n1 - l1 l2 ) , ( n2 - v1 v2 )
    chain-(2) : sp  - w1p w2p , ( w1p - l1p v1p ) , ( w2p - l2p v2p )
+--------------------------------------------+
...
+--------------------------------------------+
    ...
    constraint-(2) : w1p.M() - w2p.M() = 0 
    constraint-(3) : w1p.M() - 80.419 = 0
+--------------------------------------------*/
#include <cmath>
#include <string>
#include <iostream>
   ...
   /*--------------------------------------------------------------+
    ...
    for(int entry = 0; entry < NEvent; entry++){
       ...
       /*---------------------------------------------------+
        ...
        optm = static_cast<OptiMass::MassMinimizer *>(new OptiMass::Zprime_ww_mixed);
        optm->InitContainers();
     ...
     optm->SetInvisibleSubsystemMomenta(1,  ,  );// (i-1, PTmiss_tot.Px, PTmiss_tot.Py) for the (i=2)-th PT-conservation system
       ...
       /*-------------------------------------------------------------+
        ...
        alm_controller.UseConstraint(1, true);// (true or false) for the constraint : w1p.M() - w2p.M() = 0.
        alm_controller.UseConstraint(2, true);// (true or false) for the constraint : w1p.M() - 80.419 = 0.
       ...
       /*-----------------------+
        ...
        std::cout << "sp-mass = " << process_tree.GetSubsystemMass("sp") << std::endl;
