...
+--------------------+-----------------------+
          < Model information >
+--------------------------------------------+
#1) Model Name : 'TTbar_AB_mixed'
+--------------------------------------------+
#2) Full Decay Topology :
+--------------------------------------------+
    chain-(1) : ab - t1 t2 , ( t1 - b1 w1 , w1 - e1 v1 ) , ( t2 - b2 w2 , w2 - e2 v2 )
    chain-(2) : abj - t1j t2j , ( t1j - b1j w1j , w1j - e1j v1j ) , ( t2j - b2j w2j , w2j - e2j v2j )
+--------------------------------------------+
...
+--------------------------------------------+
    constraint-(1) : t1.M() - 173. = 0 
    constraint-(2) : t2.M() - 173. = 0 
    constraint-(3) : t1.M() - t2.M() = 0 
    constraint-(4) : w1.M() - 80.4 = 0 
    constraint-(5) : w2.M() - 80.4 = 0 
    constraint-(6) : w1.M() - w2.M() = 0 
    constraint-(7) : t1j.M() - 173. = 0 
    constraint-(8) : t2j.M() - 173. = 0 
    constraint-(9) : t1j.M() - t2j.M() = 0 
    constraint-(10) : w1j.M() - 80.4 = 0 
    constraint-(11) : w2j.M() - 80.4 = 0 
    constraint-(12) : w1j.M() - w2j.M() = 0 
    constraint-(13) : ab.M() - abj.M() = 0
+--------------------------------------------*/
#include <cmath>
#include <string>
#include <iostream>
   ...
   /*--------------------------------------------------------------+
    ...
    for(int entry = 0; entry < NEvent; entry++){
       ...
       /*---------------------------------------------------+
        ...
        optm = static_cast<OptiMass::MassMinimizer *>(new OptiMass::TTbar_AB_mixed);
        optm->InitContainers();
     ...
     optm->SetInvisibleSubsystemMomenta(1,  ,  );// (i-1, PTmiss_tot.Px, PTmiss_tot.Py) for the (i=2)-th PT-conservation system
        ...
        optm->SetMomentumValue("b1j_x",   );
        optm->SetMomentumValue("b1j_y",   );
        optm->SetMomentumValue("b1j_z",   );
        optm->SetMomentumValue("b1j_m",   );
        optm->SetMomentumValue("e1j_x",   );
        optm->SetMomentumValue("e1j_y",   );
        optm->SetMomentumValue("e1j_z",   );
        optm->SetMomentumValue("e1j_m",   );
        optm->SetMomentumValue("b2j_x",   );
        optm->SetMomentumValue("b2j_y",   );
        optm->SetMomentumValue("b2j_z",   );
        optm->SetMomentumValue("b2j_m",   );
        optm->SetMomentumValue("e2j_x",   );
        optm->SetMomentumValue("e2j_y",   );
        optm->SetMomentumValue("e2j_z",   );
        optm->SetMomentumValue("e2j_m",   );

        // Invisible Particle (Trial) Mass Input : 
        optm->SetInitInvisibleMomentum("v1_m",   );
        optm->SetInitInvisibleMomentum("v2_m",   );
        optm->SetInitInvisibleMomentum("v1j_m",   );
        optm->SetInitInvisibleMomentum("v2j_m",   );    
    
       /*-------------------------------------------------------------+
        | Constraint Function Switching :                             |
        | >> Check the definition of the functions in the list above  |
        +-------------------------------------------------------------*/
        alm_controller.UseConstraint(0, true);// (true or false) for the constraint : t1.M() - 173. = 0.
        alm_controller.UseConstraint(1, true);// (true or false) for the constraint : t2.M() - 173. = 0.
        alm_controller.UseConstraint(2, true);// (true or false) for the constraint : t1.M() - t2.M() = 0.
        alm_controller.UseConstraint(3, true);// (true or false) for the constraint : w1.M() - 80.4 = 0.
        alm_controller.UseConstraint(4, true);// (true or false) for the constraint : w2.M() - 80.4 = 0.
        alm_controller.UseConstraint(5, true);// (true or false) for the constraint : w1.M() - w2.M() = 0.
        alm_controller.UseConstraint(6, true);// (true or false) for the constraint : t1j.M() - 173. = 0.
        alm_controller.UseConstraint(7, true);// (true or false) for the constraint : t2j.M() - 173. = 0.
        alm_controller.UseConstraint(8, true);// (true or false) for the constraint : t1j.M() - t2j.M() = 0.
        alm_controller.UseConstraint(9, true);// (true or false) for the constraint : w1j.M() - 80.4 = 0.
        alm_controller.UseConstraint(10, true);// (true or false) for the constraint : w2j.M() - 80.4 = 0.
        alm_controller.UseConstraint(11, true);// (true or false) for the constraint : w1j.M() - w2j.M() = 0.
        alm_controller.UseConstraint(12, true);// (true or false) for the constraint : ab.M() - abj.M() = 0.
       ...
       /*-----------------------+
        ...
        std::cout << "ab-mass = " << process_tree.GetSubsystemMass("ab") << std::endl;
        ...
        std::cout << "abj-mass = " << process_tree.GetSubsystemMass("abj") << std::endl;
        std::cout << "t1j-mass = " << process_tree.GetSubsystemMass("t1j") << std::endl;
        std::cout << "w1j-mass = " << process_tree.GetSubsystemMass("w1j") << std::endl;
        std::cout << "v1j-mass = " << process_tree.GetSubsystemMass("v1j") << std::endl;
        std::cout << "t2j-mass = " << process_tree.GetSubsystemMass("t2j") << std::endl;
        std::cout << "w2j-mass = " << process_tree.GetSubsystemMass("w2j") << std::endl;
        std::cout << "v2j-mass = " << process_tree.GetSubsystemMass("v2j") << std::endl;
        std::cout << "==================================================" << std::endl;
