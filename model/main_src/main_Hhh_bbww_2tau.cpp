...
+--------------------+-----------------------+
          < Model information >
+--------------------------------------------+
#1) Model Name : 'Hhh_bbww_2tau'
+--------------------------------------------+
#2) Full Decay Topology :
+--------------------------------------------+
    chain-(1) : H - h1 h2 , ( h1 - b1 b2 ) , ( h2 - w1 w2 , ( w1 - tau1 vt1 , ( tau1 - l1 vl1 vt2) ) , ( w2 - tau2 vt3 , ( tau2 - l2 vl2 vt4 ) ) )
+--------------------------------------------+
...
+--------------------------------------------+
    ...
    constraint-(3) : tau1.M() - 1.777 = 0 
    constraint-(4) : tau2.M() - 1.777 = 0 
    constraint-(5) : h1.M() - h2.M() = 0
+--------------------------------------------*/
#include <cmath>
#include <string>
#include <iostream>
   ...
   /*--------------------------------------------------------------+
    ...
    for(int entry = 0; entry < NEvent; entry++){
       ...
       /*---------------------------------------------------+
        ...
        optm = static_cast<OptiMass::MassMinimizer *>(new OptiMass::Hhh_bbww_2tau);
        optm->InitContainers();
        ...
        */
        ...
        optm->SetMomentumValue("vt4_x",   );
        optm->SetMomentumValue("vt4_y",   );
        optm->SetMomentumValue("vt4_z",   );
        optm->SetMomentumValue("vt4_m",   );
        optm->SetMomentumValue("vt3_x",   );
        optm->SetMomentumValue("vt3_y",   );
        optm->SetMomentumValue("vt3_z",   );
        optm->SetMomentumValue("vt3_m",   );
       ...
       /*-------------------------------------------------------------+
        ...
        alm_controller.UseConstraint(2, true);// (true or false) for the constraint : tau1.M() - 1.777 = 0.
        alm_controller.UseConstraint(3, true);// (true or false) for the constraint : tau2.M() - 1.777 = 0.
        alm_controller.UseConstraint(4, true);// (true or false) for the constraint : h1.M() - h2.M() = 0.
       ...
       /*-----------------------+
        ...
        std::cout << "tau2-mass = " << process_tree.GetSubsystemMass("tau2") << std::endl;
