...
+--------------------+-----------------------+
          < Model information >
+--------------------------------------------+
#1) Model Name : 'ww_mixed'
+--------------------------------------------+
#2) Full Decay Topology :
+--------------------------------------------+
    chain-(1) : s - w1 w2 , ( w1 - l1 v1 ), ( w2 - l2 v2 )
...
+--------------------------------------------+
    ...
    constraint-(3) : w1p.M() - w2p.M() = 0 
    constraint-(4) : w1p.M() - 80.419 = 0
+--------------------------------------------*/
#include <cmath>
#include <string>
#include <iostream>
   ...
   /*--------------------------------------------------------------+
    ...
    for(int entry = 0; entry < NEvent; entry++){
       ...
       /*---------------------------------------------------+
        ...
        optm = static_cast<OptiMass::MassMinimizer *>(new OptiMass::ww_mixed);
        optm->InitContainers();
     ...
     optm->SetInvisibleSubsystemMomenta(1,  ,  );// (i-1, PTmiss_tot.Px, PTmiss_tot.Py) for the (i=2)-th PT-conservation system
       ...
       /*-------------------------------------------------------------+
        ...
        alm_controller.UseConstraint(2, true);// (true or false) for the constraint : w1p.M() - w2p.M() = 0.
        alm_controller.UseConstraint(3, true);// (true or false) for the constraint : w1p.M() - 80.419 = 0.
