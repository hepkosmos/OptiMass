...
+--------------------+-----------------------+
          < Model information >
+--------------------------------------------+
#1) Model Name : 'Hhh_mixed'
+--------------------------------------------+
#2) Full Decay Topology :
+--------------------------------------------+
    chain-(1) : H - h1 h2 , ( h1 - b1 b2 ) , ( h2 - w1 w2 , ( w1 - e1 v1 ) , ( w2 - e2 v2 ) )
    chain-(2) : Hp - h1p h2p , ( h1p - b1p b2p ) , ( h2p - w1p w2p , ( w1p - e1p v1p ) , ( w2p - e2p v2p ) )
+--------------------------------------------+
...
+--------------------------------------------+
    ...
    constraint-(4) : h1p.M() - 125. = 0 
    constraint-(5) : h2p.M() - 125. = 0 
    constraint-(6) : h1p.M() - h2p.M() = 0 
    constraint-(7) : H.M() - Hp.M() = 0
+--------------------------------------------*/
#include <cmath>
#include <string>
#include <iostream>
   ...
   /*--------------------------------------------------------------+
    ...
    for(int entry = 0; entry < NEvent; entry++){
       ...
       /*---------------------------------------------------+
        ...
        optm = static_cast<OptiMass::MassMinimizer *>(new OptiMass::Hhh_mixed);
        optm->InitContainers();
     ...
     optm->SetInvisibleSubsystemMomenta(1,  ,  );// (i-1, PTmiss_tot.Px, PTmiss_tot.Py) for the (i=2)-th PT-conservation system
       ...
       /*-------------------------------------------------------------+
        ...
        alm_controller.UseConstraint(3, true);// (true or false) for the constraint : h1p.M() - 125. = 0.
        alm_controller.UseConstraint(4, true);// (true or false) for the constraint : h2p.M() - 125. = 0.
        alm_controller.UseConstraint(5, true);// (true or false) for the constraint : h1p.M() - h2p.M() = 0.
        alm_controller.UseConstraint(6, true);// (true or false) for the constraint : H.M() - Hp.M() = 0.
       ...
       /*-----------------------+
        ...
        std::cout << "h1p-mass = " << process_tree.GetSubsystemMass("h1p") << std::endl;
        std::cout << "h2p-mass = " << process_tree.GetSubsystemMass("h2p") << std::endl;
